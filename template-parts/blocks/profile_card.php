...
$pha_mem = get_field('phacilitate_board_members');


$post_per_page = get_field('post_per_page');

$text_link = get_field('text_link');
$heading_member = get_field('heading_members');
$sub_heading_member = get_field('sub_heading_members');
?>
        ...
        <div class="container">
            <div class="profile_card_inner">
                ...
                <div class="latest_post_list ">
                  ...
                  <?php if ($heading_member || $sub_heading_member) { ?>
                    <div class="heading_section">
                        <?php if ($heading_member) { ?>
                            <div class="heading h2">
                                <?php echo $heading_member ?>
                            </div>
                        <?php } ?>
                        <?php if ($sub_heading_member) { ?>
                            <div class="sub_heading h6">
                                <?php echo $sub_heading_member ?>
                            </div>
                        <?php } ?>
                    </div>
                <?php } ?>
                
                  <div class="latest_post_list Members">
                    <?php
                    if ($post_list_type == 'manual') {
                        $args = array(
                            'post__in'   => $pha_mem,
                            'orderby'   => 'post__in',
                            'post_type'        => 'pha_member',
                            'post_status'      => 'publish',
                            'posts_per_page' => -1,
                            // 'tax_query' => array(
                            //     array(
                            //       'taxonomy' => 'pha_member_category',
                            //       'terms' => $category,
                            //     ),),                           
                        );
                    } else {
                    ...
                    <?php if ($the_query->have_posts()) { ?>
                        ...
                        <div class="three_column_grid team_section">
                            <div class="team_section_inner">
                                ...
                                <?php while ($the_query->have_posts()) {
                                    $the_query->the_post();  ?>
                                    <?php get_template_part('template-parts/content', 'pha_member'); ?>
                                <?php } ?>
                            </div>
                        </div>
                        <?php if ($the_query->max_num_pages > 1 && $post_list_type != 'manual') { ?>
