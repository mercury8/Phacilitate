<?php 
    $ondemand_posts_id = get_query_var( 'postId' );
    $title = get_the_title( $ondemand_posts_id );
    $link = get_the_permalink( $ondemand_posts_id );
    $image = get_the_post_thumbnail_url( $ondemand_posts_id, 'full' );
    $time = get_field('time', $ondemand_posts_id);
    $location = get_field('location',$ondemand_posts_id);
    $date = get_the_date( 'j M Y',$ondemand_posts_id );
?>


<div class="webinars_list_upcoming_ondemand_list_item">
